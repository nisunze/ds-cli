...
use super::{LANE_ARG, PROJECT_ARG, REASON_ARG, TRANSFORMER_ARG};
...
service to retire the named transformers in the project --project names; the \
saved selection is never read. A retired transformer leaves every listing, report, tile run and \
...
a destructive paired-application action.",
    ...
    args: &[PROJECT_ARG, TRANSFORMER_ARG, REASON_ARG, LANE_ARG],
    output: "\
...
`failed`) with its message.",
    examples: &[Example {
        command: "ds design transformer retire --project <id> --transformer TX-1 --reason \"superseded by the 2026 survey\" --yes",
        note: "Inspect first with `ds design transformer inventory --project <id> --transformer TX-1`.",
        runnable: false,
    }],
