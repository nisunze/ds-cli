...
pub static COMMAND: Command = Command {
    ...
    purpose: "\
Restores the native user, uses the project --project names (the saved selection is never read), \
...
No Desktop descriptor, URL, body or action override is accepted.",
    ...
    args: &[PROJECT_ARG, TYPE_ARG, LANE_ARG],
    output: "\
...
pub fn run(inputs: &Inputs, _context: &Context) -> Result<Value, Failure> {
    ...
    let headless =
        ds_cli_auth::tile_preflight(inputs.require("lane")?, inputs.require("project")?, kind)?;
