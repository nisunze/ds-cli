...
//! lane under `DS_LIVE_LANE` (default `stable`) and the project it reads in
//! `DS_LIVE_PROJECT`:
//!
//! ```text
//! DS_NATIVE_CLIENT_PROFILE_BUNDLE=/abs/catalog.json DS_LIVE_PROJECT=<id> \
...
//! `requireProjectAccess` and `listVectorTiles` for the named project, and
...
#[test]
#[ignore = "needs DS_NATIVE_CLIENT_PROFILE_BUNDLE and a signed-in native identity and DS_LIVE_PROJECT"]
fn a_signed_in_identity_reads_its_reference_catalogue() {
    let lane = std::env::var("DS_LIVE_LANE").unwrap_or_else(|_| "stable".to_owned());
    let project = std::env::var("DS_LIVE_PROJECT").expect("DS_LIVE_PROJECT names the project");
    let rows = data_distribution(&lane, &project, &DataDistributionRequest::ListDatasets {})
