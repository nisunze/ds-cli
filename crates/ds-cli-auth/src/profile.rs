...
fn load_path(
    ...
    ClientProfile::validate(ClientProfileInput {
        ...
        project_form_editor_action: entry.project_forms.settings_editor_action,
        ...
        survey_control_routes: entry.survey_control,
        styles_method: entry.styles.method,
        styles_path: entry.styles.path,
        styles_action: entry.styles.action,
        printing_method: entry.printing.method,
        printing_path: entry.printing.path,
        printing_actions: entry.printing.actions,
        layers_method: entry.layers.method,
        layers_path: entry.layers.path,
        layers_actions: entry.layers.actions,
        ...
        design_attachments_method: entry.design_attachments.method,
        design_attachments_path: entry.design_attachments.path,
        design_attachments_actions: entry.design_attachments.actions,
...
mod tests {
        ...
        serde_json::to_vec(&json!({
            ...
            "profiles": {
                "stable": {
                    ...
                    "transformer_context": { "method": "POST", "path": "/report", "action": "get_transformers_data", "fields": "full" },
                    ...
                    "tiles": { "method": "POST", "path": "/api/v1/tiles", "actions": ["status", "preflight", "generate", "list", "add", "remove", "global_catalog", "global_list", "global_generate", "global_status"] },
                    ...
                    "design_attachments": {"method":"POST","path":"/api/v1/design/attachments","actions":["list","get","start_upload","finalize_upload","download","archive","archive_revision"]},
                    "design_versions": {"method":"POST","path":"/api/v1/design/versions","actions":["list_versions","get_version","get_head","create_version","restore_version","revise_version","freeze_version","list_version_events","link_attachment","unlink_attachment"]},
    ...
    fn v12_fixed_calls_are_required_and_exact() {
        ...
        let mut escaped: serde_json::Value = serde_json::from_slice(&fixture(false)).unwrap();
        escaped["profiles"]["stable"]["project_forms"]["settings_editor_action"] = json!("editor");
        let escaped = serde_json::to_vec(&escaped).unwrap();
        with_fixture(&escaped, |path| {
            assert_eq!(
                load_path(
