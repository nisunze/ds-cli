...
pub static PROJECT_USE_COMMAND: Command = Command {
    ...
    purpose: "Fetches all three fresh project buckets, requires an exact visible ds_project match, and atomically saves a UID/lane/client-audience-fenced local context. A project ID by itself is never accepted as authority. The saved context is a device-local convenience only: no current ds command reads it; every project command names its project with --project.",
    effect: Effect::LocalAuthState,
    authority: Authority::HeadlessUser,
    execution: Execution::Sync,
...
pub static PROJECT_STATUS_COMMAND: Command = Command {
    ...
    purpose: "Restores the native user, then reads only a context bound to the same UID, canonical email, lane, and credential audience. It never reads the Desktop project. No current ds command reads this context; every project command names its project with --project.",
    effect: Effect::LocalAuthState,
    authority: Authority::HeadlessUser,
    execution: Execution::Sync,
    args: &[LANE],
