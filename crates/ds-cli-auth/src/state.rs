...
mod tests {
    use super::*;
    use crate::test_support::{FixtureTransport, NOW, REFRESH, SIGN_IN, profile};
    use ds_client_core::Client;
    ...
    fn real_file_adapter_round_trips_core_rotation_and_logout() {
        ...
        let mut signed_in = Client::new(
            profile(),
            FixtureTransport::with_sign_in(SIGN_IN),
            store(&root),
        );
        let user = signed_in
        ...
        let mut restored = Client::new(
            profile(),
            FixtureTransport::with_refresh(REFRESH),
            store(&root),
        );
