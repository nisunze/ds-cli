...
fn root_help_is_cheap() {
    ...
    // `style` domain landed and from 1,600 when `tile` landed: a domain costs
    // root help one line forever, which is the trade
    // `root_help_scales_with_domains_not_commands` exists to price. The flat
    // ceiling stays at that scaled one (800 + 80 × 11 domains) so it still
    // catches prose creeping into the frame.
