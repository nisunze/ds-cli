...
pub static CATALOG_COMMAND: Command = Command {
    ...
    contract: 1,
    summary: "Reconcile published Rwanda geographic sources into Firestore (needs --yes).",
...
pub static INSTALL_COMMAND: Command = Command {
    ...
    purpose: "Downloads all published bundles or explicit repeated --resource selections, verifies both compressed and expanded digests, and builds each local SQLite RTree. --max-download-mib bounds transfer size; skipped or missing datasets never block maps or printing.",
    chapter: Chapter::Data,
