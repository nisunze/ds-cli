...
pub static EXPORT_COMMAND: Command = Command {
    ...
    purpose:"Discover all recipe fields with map.print.schema. The matching signed-in desktop captures every project MV source and selected geographic context. The command kernel owns exact scope, paper and canonical output identity. Native Reporter renders a custom-area page, ordered district pages, or one combined district overview in one PDF. Project Control retains the local artifact and source warnings; only the selected paper is replaced. Completed printouts attach automatically through the existing report-artifact channel; disconnected or failed publication remains pending locally. Cloud rendering never occurs.",
