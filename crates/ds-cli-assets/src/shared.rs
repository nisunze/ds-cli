...
const fn reference_refusals()
-> [Refusal; 3 + ds_cli_auth::PROJECT_STATUS_COMMAND.refusals.len() + 8] {
    let base = refusals();
    let mut out = [EXTRA; 3 + ds_cli_auth::PROJECT_STATUS_COMMAND.refusals.len() + 8];
    let mut i = 0;
    while i < base.len() {
        out[i] = base[i];
        i += 1;
    }
    out[i] = crate::INVALID_REFERENCE_FORM;
    out[i + 1] = crate::INVALID_EXTERNAL_REFERENCE;
    out[i + 2] = crate::ASSET_REFERENCE_CONFLICT;
    out[i + 3] = crate::UNKNOWN_FOLDER;
    out[i + 4] = crate::ASSET_CLASS_FORBIDDEN;
    out[i + 5] = crate::ASSET_RULE_REFUSED;
    out[i + 6] = crate::ASSET_REQUEST_INVALID;
    out[i + 7] = crate::ASSETS_SERVICE_FAILED;
    out[i + 8] = crate::CONFIRMATION_REQUIRED;
    out
}
