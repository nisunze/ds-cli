...
ds solar results read --source <closed-batch-dir> --project <id> --run-id <id> --city <context> --section <name>
ds solar dashboard compose --source <closed-batch-dir> --project <id> --run-id <id> --city <context> --section site --out <new-dir>
...
`solar results read` contract 2 reads a bounded section from a closed local batch.
Pass `--source`, `--project`, `--run-id` and `--city`; the Rust owner verifies
