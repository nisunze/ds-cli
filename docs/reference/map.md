...
The map Working set is a separate local preference. `map design pin` reads or changes
...
ds map design pin --mode unpin --transformer agasharu --output json
ds map design pin --mode clear --output json
```
...
## Rendered pages are immutable report artifacts

The DS cartography renderer normally attaches its outputs directly. An already
rendered, operator-reviewed PDF/PNG/JPEG can also be attached explicitly:
