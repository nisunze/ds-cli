...
`ds desktop data rwanda install` without `--resource` downloads and indexes **every** published dataset this computer does not already hold, smallest first — including the two calculation datasets (`rw-admin-villages`, `rwanda-dem-10m`) that used to be installable only by name, which is why an operator kept being told to run `--resource rwanda-dem-10m` by hand. It is safe to run at any time, including in the middle of preparing a report or a map: it spends disk and nothing else, changes nothing in the cloud, and `ds desktop data rwanda remove` puts every byte back. Nobody has to ask for it.
