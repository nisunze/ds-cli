...
6. **The first two typed mutations and the structure list** (D1/D2 of the delivery path), all `authority none`, `requires server`, MCP parity through the descriptors:
   - `ds dsgrid structure describe --model <id> --structure <id|number> --text "…" | --clear [--revision] --dry-run|--yes` → engine `describe_structure` (new journaled command; `StructureRow.description`, one trimmed line ≤ 500 chars, the placed structure's own text beside the library type's description). *Decision:* the description is a canonical column of the structures table (16th, nullable), which widens the package schema — packages written before this build are refused `package_decode_failed` with the remedy "re-convert from the PLS-CADD workspace" (the same event as `delivery_phase` on 08-29 and granular tags on 09-02). The owner's Windows working copy `local-e9b0ccbf92d7447b` predates it and is re-imported, not migrated. Contract 02 exports the text into the DON 57 structure record on sync.
