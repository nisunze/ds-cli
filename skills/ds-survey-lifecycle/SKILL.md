...
| “Report on the surveyed assets, with their photos.” | Every field value and each entry's photo references, as the map shows them. | `survey.entries.read --project --form` (`--out` for a whole form); [read data](references/read-data.md). |
