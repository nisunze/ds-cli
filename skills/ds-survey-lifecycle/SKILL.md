...
| “Report on the surveyed assets, with their photos.” | Every field value and each entry's photo references, as the map shows them. | `survey.local.status` (what is held), `survey.entries.read --project --form` (`--out` for a whole form), then `survey.photo.fetch --from <that file> --out-dir` (thumbnails; `--original` only where detail matters); [read data](references/read-data.md). |
